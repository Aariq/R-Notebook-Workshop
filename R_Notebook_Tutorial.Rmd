...
A note on filenames:  When generating a report as a pdf, part of the process doesn't like filenames with spaces in them.  This isn't *always* a problem, but it's best to avoid spaces in .rmd filenames to avoid problems.
    
## Reports

Generating pdf, word, or html documents is done using the "knit" button (which, by default says "preview").  This invokes functions from the R package `knitr` to produce the desired output.  For pdf, that involves LaTeX, so you must have this installed on your computer to generate pdf output.
